...
1) 服务端部署（推荐生产）
   ...
   Linux systemd 示例（改路径后）：
     [Service]
     WorkingDirectory=/opt/oa
     ExecStart=/usr/bin/java -jar /opt/oa/oa-backend-1.0.0.jar --server.port=8086
     Restart=on-failure
...
4) Linux
   ...
   （或不装 Java，用 linux-x64 原生包：./oa/bin/oa）

浏览器： http://127.0.0.1:8086
默认账号 `admin / admin123`。
数据目录：当前目录 data/
