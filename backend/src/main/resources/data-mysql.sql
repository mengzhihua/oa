...
INSERT IGNORE INTO org_dept (code, name, parent_id, sort, status, path)
    VALUES ('HQ', '集团总部', NULL, 0, 1, '/HQ');
INSERT IGNORE INTO org_dept (code, name, parent_id, sort, status, path)
...
INSERT IGNORE INTO org_position (code, name, level, dept_id)
...
INSERT IGNORE INTO org_job_grade (code, name, level) VALUES
    ('G1', '初级', 1), ('G2', '中级', 2), ('G3', '高级', 3);
...
INSERT IGNORE INTO wf_definition (code, name, form_schema_json, status, version) VALUES
...
INSERT IGNORE INTO wf_node (definition_id, seq, name, approver_type, multi_mode, condition_json)
SELECT d.id, 1, '部门主管', 'DEPT_LEADER', 'ANY', NULL FROM wf_definition d;
INSERT IGNORE INTO wf_node (definition_id, seq, name, approver_type, multi_mode, condition_json)
SELECT d.id, 2, '人力资源', 'HR', 'ANY', '{"field":"days","op":">","value":3}'
FROM wf_definition d WHERE d.code IN ('LEAVE', 'BUSINESS_TRIP');
INSERT IGNORE INTO wf_node (definition_id, seq, name, approver_type, multi_mode, condition_json)
SELECT d.id, 2, '财务审核', 'FINANCE', 'ALL', NULL
FROM wf_definition d WHERE d.code = 'EXPENSE';

INSERT IGNORE INTO att_shift (
...
INSERT IGNORE INTO att_holiday (holiday_date, name, type) VALUES
    ('2026-01-01', '元旦', 'HOLIDAY'), ('2026-02-17', '春节', 'HOLIDAY'),
    ('2026-02-18', '春节', 'HOLIDAY'), ('2026-02-19', '春节', 'HOLIDAY'),
    ('2026-04-04', '清明节', 'HOLIDAY'), ('2026-05-01', '劳动节', 'HOLIDAY'),
    ('2026-06-19', '端午节', 'HOLIDAY'), ('2026-09-25', '中秋节', 'HOLIDAY'),
    ('2026-10-01', '国庆节', 'HOLIDAY'), ('2026-10-02', '国庆节', 'HOLIDAY'),
    ('2026-10-03', '国庆节', 'HOLIDAY'), ('2026-10-04', '国庆节', 'HOLIDAY'),
    ('2026-10-05', '国庆节', 'HOLIDAY'), ('2026-10-06', '国庆节', 'HOLIDAY'),
    ('2026-10-07', '国庆节', 'HOLIDAY'), ('2026-02-14', '春节调休', 'WORKDAY'),
    ('2026-02-28', '春节调休', 'WORKDAY'), ('2026-05-09', '劳动节调休', 'WORKDAY'),
...
INSERT IGNORE INTO pay_insurance_rule (
...
) VALUES ('北京', 0.08, 0.16, 0.02, 0.09, 0.005, 0.005, 0.002, 0.008,
          0.12, 0.12, 5869, 31884, 2320, 31884);

INSERT IGNORE INTO pay_tax_bracket (
    level_no, lower_bound, upper_bound, rate, quick_deduction
) VALUES
    (1, 0, 36000, 0.03, 0), (2, 36000.01, 144000, 0.10, 2520),
    (3, 144000.01, 300000, 0.20, 16920), (4, 300000.01, 420000, 0.25, 31920),
    (5, 420000.01, 660000, 0.30, 52920), (6, 660000.01, 960000, 0.35, 85920),
